...
impl BlockchainDb {
    ...
    fn new_db(meta: BlockchainDbMeta, cache_path: Option<PathBuf>, skip_check: bool) -> Self {
        tracing::trace!(target : "forge::cache", cache=?cache_path, "initialising blockchain db");
        ...
        let cache = cache_path
            ...
            .and_then(|p| {
                JsonBlockCacheDB::load(p).ok().filter(|cache| {
                    ...
                    if meta != *existing {
                        tracing::warn!(target : "cache", "non-matching block metadata");
                        false
                    } else {
                        true
                    }
...
impl JsonBlockCacheDB {
    ...
    pub fn load(path: impl Into<PathBuf>) -> eyre::Result<Self> {
        let path = path.into();
        tracing::trace!(target : "cache", ?path, "reading json cache");
        let contents = std::fs::read_to_string(&path).map_err(|err| {
            tracing::warn!(?err, ?path, "Failed to read cache file");
            err
        })?;
        let data = serde_json::from_str(&contents).map_err(|err| {
            tracing::warn!(target : "cache", ?err, ?path, "Failed to deserialize cache data");
            err
        })?;
        Ok(Self {
    ...
    pub fn flush(&self) {
        ...
        tracing::trace!(target: "cache", "saving json cache");
        ...
        let file = match fs::File::create(path) {
            Ok(file) => file,
            Err(e) => {
                return tracing::warn!(target: "cache", %e, "Failed to open json cache for writing")
            }
        };

        let mut writer = BufWriter::new(file);
        if let Err(e) = serde_json::to_writer(&mut writer, &self.data) {
            return tracing::warn!(target: "cache", %e, "Failed to write to json cache");
        }
        if let Err(e) = writer.flush() {
            return tracing::warn!(target: "cache", %e, "Failed to flush to json cache");
        }

        tracing::trace!(target: "cache", "saved json cache");
    }
}
...
impl Drop for FlushJsonBlockCacheDB {
    fn drop(&mut self) {
        tracing::trace!(target: "fork::cache", "flushing cache");
        self.0.flush();
        tracing::trace!(target: "fork::cache", "flushed cache");
    }
}
