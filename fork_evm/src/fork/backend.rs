            ...
            Entry::Vacant(entry) => {
                ...
                let fut = Box::pin(async move {
                    ...
                    let block_hash = match block {
                        ...
                        Err(err) => {
                            tracing::error!(target: "backendhandler", ?err, ?number, "failed to get block");
                            Err(err)
                        }
                    };
...
impl DatabaseRef for SharedBackend {
    ...
    fn basic(&self, address: Address) -> Result<Option<AccountInfo>, Self::Error> {
        ...
        self.do_get_basic(address).map_err(|err| {
            tracing::error!(target: "sharedbackend",  ?err, ?address,  "Failed to send/recv `basic`");
            if err.is_possibly_non_archive_node_error() {
                tracing::error!(target: "sharedbackend", "{NON_ARCHIVE_NODE_WARNING}");
            }
            err
        })
    }
    ...
    fn storage(&self, address: Address, index: U256) -> Result<U256, Self::Error> {
        ...
        match self.do_get_storage(address, index).map_err(|err| {
            tracing::error!( target: "sharedbackend", ?err, ?address, ?index, "Failed to send/recv `storage`");
            if err.is_possibly_non_archive_node_error() {
                tracing::error!(target: "sharedbackend", "{NON_ARCHIVE_NODE_WARNING}");
            }
          err
    ...
    fn block_hash(&self, number: U256) -> Result<B256, Self::Error> {
        ...
        match self.do_get_block_hash(number).map_err(|err| {
            tracing::error!(target: "sharedbackend",?err, ?number, "Failed to send/recv `block_hash`");
            if err.is_possibly_non_archive_node_error() {
                tracing::error!(target: "sharedbackend", "{NON_ARCHIVE_NODE_WARNING}");
            }
            err
