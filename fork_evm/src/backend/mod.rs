...
use crate::constants::{ARBITRUM_SENDER, OPTIMISM_SYSTEM_ADDRESS, SYSTEM_TRANSACTION_TYPE};
...
impl Backend {
    ...
    pub fn new(forks: MultiFork, fork: Option<CreateFork>) -> Self {
        tracing::trace!(target: "backend", forking_mode=?fork.is_some(), "creating executor backend");
        ...
        tracing::trace!(target: "backend", forking_mode=? backend.active_fork_ids.is_some(), "created executor backend");
    ...
    pub fn set_test_contract(&mut self, acc: Address) -> &mut Self {
        tracing::trace!(?acc, "setting test account");
    ...
    pub fn set_caller(&mut self, acc: Address) -> &mut Self {
        tracing::trace!(?acc, "setting caller account");
    ...
    pub fn set_spec_id(&mut self, spec_id: SpecId) -> &mut Self {
        tracing::trace!("setting precompile id");
    ...
    pub fn set_snapshot_failure(&self, has_snapshot_failure: bool) {
        self.inner
            ...
            .store(has_snapshot_failure, Ordering::Relaxed);
    }
    ...
    fn prepare_init_journal_state(&mut self) -> Result<(), DatabaseError> {
        ...
        for fork in self.inner.forks_iter_mut() {
            ...
            for loaded_account in loaded_accounts.iter().copied() {
                tracing::trace!(?loaded_account, "replacing account on init");
                let fork_account = Database::basic(&mut fork.db, loaded_account)?
                    .ok_or(DatabaseError::MissingAccount(loaded_account))?;
                let init_account = journaled_state
                    .state
                    .get_mut(&loaded_account)
                    .expect("exists; qed");
    ...
    pub fn replay_until(
        ...
        tracing::trace!(?id, ?tx_hash, "replay until transaction");

        let fork_id = self.ensure_fork_id(id)?.clone();

        let fork = self.inner.get_fork_by_id_mut(id)?;
        let full_block = fork.db.db.get_full_block(BlockNumber::Number(
            env.block.number.to_ethers().as_u64().into(),
        ))?;
        ...
        for tx in full_block.transactions.into_iter() {
            ...
            tracing::trace!(tx=?tx.hash, "committing transaction");
...
impl DatabaseExt for Backend {
    ...
    fn revert(
        ...
        tracing::trace!(?id, "revert snapshot");
        if let Some(mut snapshot) = self.inner.snapshots.remove_at(id) {
            ...
            tracing::trace!(target: "backend", "Reverted snapshot {}", id);

            Some(journaled_state)
        } else {
            tracing::warn!(target: "backend", "No snapshot to revert for {}", id);
            None
        }
    }

    fn create_fork(&mut self, fork: CreateFork) -> eyre::Result<LocalForkId> {
        tracing::trace!("create fork");
    ...
    fn create_fork_at_transaction(
        ...
        tracing::trace!(?transaction, "create fork at transaction");
        let id = self.create_fork(fork)?;
    ...
    fn select_fork(
        ...
        if let Some(active) = self.active_fork_mut() {
            ...
            tracing::trace!("recording fork init journaled_state");
...

        {
            ...
            if !fork.journaled_state.state.contains_key(&caller) {
                ...
                fork.journaled_state
                    .state
                    .insert(caller, caller_account.into());
            }
    ...
    fn roll_fork(
        ...
        tracing::trace!(?id, ?block_number, "roll fork");
        let id = self.ensure_fork(id)?;
        let (fork_id, backend, fork_env) = self
            .forks
            .roll_fork(self.inner.ensure_fork_id(id).cloned()?, block_number.to())?;
    ...
    fn roll_fork_to_transaction(
        ...
        tracing::trace!(?id, ?transaction, "roll fork to transaction");
        let id = self.ensure_fork(id)?;
        ...
        env.block.number = block
            .number
            .map(|n| n.to_alloy())
            .unwrap_or(fork_block)
            .to();
    ...
    fn transact<I: Inspector<Backend>>(
        ...
        tracing::trace!(?maybe_id, ?transaction, "execute transaction");
    ...
    fn diagnose_revert(
        ...
        if !active_fork.is_contract(callee) && !is_contract_in_state(journaled_state, callee) {
            ...
            for (id, fork) in self.inner.forks_iter().filter(|(id, _)| *id != active_id) {
                tracing::trace!(?id, address=?callee, "checking if account exists");
    ...
    fn remove_persistent_account(&mut self, account: &Address) -> bool {
        tracing::trace!(?account, "remove persistent account");
        self.inner.persistent_accounts.remove(account)
    }

    fn add_persistent_account(&mut self, account: Address) -> bool {
        tracing::trace!(?account, "add persistent account");
        self.inner.persistent_accounts.insert(account)
    }

    fn allow_cheatcode_access(&mut self, account: Address) -> bool {
        tracing::trace!(?account, "allow cheatcode access");
        self.inner.cheatcode_access_accounts.insert(account)
    }

    fn revoke_cheatcode_access(&mut self, account: Address) -> bool {
        tracing::trace!(?account, "revoke cheatcode access");
...
fn merge_journaled_state_data(
    ...
    if let Some(mut acc) = active_journaled_state.state.get(&addr).cloned() {
        tracing::trace!(?addr, "updating journaled_state account data");
...
fn merge_db_account_data<ExtDB: DatabaseRef>(
    ...
    tracing::trace!(?addr, "merging database data");
